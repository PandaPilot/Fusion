...
void FusionAhrsInitialise(FusionAhrs* const ahrs) {
  const FusionAhrsSettings settings = {
    ...
    .accelerationRejection = 40.0F,
    .magneticRejection = 90.0F,
    .recoveryTriggerPeriod = 50,
  };
...
void FusionAhrsSetSettings(FusionAhrs* const ahrs, const FusionAhrsSettings* const settings) {
  ...
  if ((settings->gain == 0.0f) ||
      (settings->recoveryTriggerPeriod == 0)) {  // disable acceleration and magnetic rejection
                                                 // features if gain is zero
...
#define Q ahrs->quaternion.element
  ...
  // Calculate accelerometer feedback (with magnitude-based weighting)
  FusionVector halfAccelerometerFeedback = FUSION_VECTOR_ZERO;
  ahrs->accelerometerIgnored = true;

  if (FusionVectorIsZero(accelerometer) == false) {
    // --- Magnitude-based weight (1 when |a|≈g, fades to 0 as |a| deviates) ---
    const float mag = FusionVectorMagnitude(accelerometer);
    const float g_ref = 1.0f;  // <-- Set to 9.80665f if your accelerometer is in m/s^2
    float mag_err_norm = fabsf(mag - g_ref) / g_ref;
    mag_err_norm = Clamp(mag_err_norm, 0.0f, 1.0f);
    const float mag_weight = 1.0f - mag_err_norm;

    // Calculate accelerometer feedback scaled by 0.5 (as per existing Feedback/HalfGravity logic)
    ...
    ahrs->accelerationCorrection = false;

    // Apply accelerometer feedback (weighted by magnitude confidence)
    if (ahrs->accelerometerIgnored == false) {
      halfAccelerometerFeedback =
          FusionVectorMultiplyScalar(ahrs->halfAccelerometerFeedback, mag_weight);
      ahrs->accelerationCorrection = (mag_weight > 0.0f);
    }
  }
  ...
  switch (ahrs->settings.convention) {
    ...
    case FusionConventionEnu: {
      const FusionVector halfGravity = { .axis = {
                                         ...
                                         } };  // third column of transposed rotation matrix scaled
                                               // by 0.5
      return halfGravity;
    }
  ...
  switch (ahrs->settings.convention) {
    case FusionConventionNwu: {
      const FusionVector halfMagnetic = { .axis = {
                                              ...
                                              .z = Q.y * Q.z - Q.w * Q.x,
                                          } };  // second column of transposed rotation matrix
                                                // scaled by 0.5
      return halfMagnetic;
    }
    case FusionConventionEnu: {
      const FusionVector halfMagnetic = { .axis = {
                                          ...
                                          } };  // first column of transposed rotation matrix scaled
                                                // by -0.5
      return halfMagnetic;
    }
    case FusionConventionNed: {
...
void FusionAhrsUpdateExternalHeading(FusionAhrs* const ahrs, const FusionVector gyroscope,
                                     const FusionVector accelerometer, const float heading,
                                     const float deltaTime) {
#define Q ahrs->quaternion.element
...
#define A ahrs->accelerometer.axis
  ...
  FusionVector accelerometer = { .axis = {
                                     .x = 2.0f * ((qwqw - 0.5f + Q.x * Q.x) * A.x +
                                                  (qxqy - qwqz) * A.y + (qxqz + qwqy) * A.z),
                                     .y = 2.0f *
